...
class ReviewCoachLectureControllerTest extends AbstractControllerTest {
    private static final String REVIEW_COACH_LECTURE_URL = "/api/review/coach/lecture/";

    @WithMockKnowllyUser
    @Test
    public void 사용자_리뷰_작성_테스트() throws Exception {
        final String json = "{" +
                "   \"content\": \"테스트3는 좋은 코치입니다!\"," +
                "   \"public\": true" +
                "}";

        mockMvc.perform(
                        post(REVIEW_COACH_LECTURE_URL + 5)
                ...
                .andExpect(MockMvcResultMatchers.jsonPath("$.data.reviewee.id").value(3))
                .andExpect(MockMvcResultMatchers.jsonPath("$.data.content").value("테스트3는 좋은 코치입니다!"))
